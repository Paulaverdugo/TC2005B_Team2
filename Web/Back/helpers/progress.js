...
export async function getProgressGadget() {
    const db = await connectDB();
    const [res, fields] = await db.execute(
...
export async function addProgress(level_achieved, user_name, player_type, life_points) {
    ...
    const [res] = await db.execute(
        `INSERT INTO scollective.USER_INFO(level_achieved, user_name, player_type, life_points) VALUES(\'${level_achieved}\', \'${user_name}\', \'${player_type}\', \'${life_points}\')'`
    );
